...
public class SchedulerFactoryBean extends SchedulerAccessor implements
	...
	public void setSchedulerFactoryClass(Class schedulerFactoryClass) {
		if (schedulerFactoryClass == null
				|| !SchedulerFactory.class
						.isAssignableFrom(schedulerFactoryClass)) {
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (this.nonTransactionalDataSource != null) {
			...
			configTimeNonTransactionalDataSourceHolder
					.set(this.nonTransactionalDataSource);
		}

		// Get Scheduler instance from SchedulerFactory.
		try {
			this.scheduler = createScheduler(schedulerFactory,
					this.schedulerName);
			populateSchedulerContext();

			if (!this.jobFactorySet
					&& !(this.scheduler instanceof RemoteScheduler)) {
			...
			if (this.jobFactory != null) {
				if (this.jobFactory instanceof SchedulerContextAware) {
					((SchedulerContextAware) this.jobFactory)
							.setSchedulerContext(this.scheduler.getContext());
				}
	...
	private void initSchedulerFactory(SchedulerFactory schedulerFactory)
		...
		if (this.configLocation != null) {
			if (logger.isInfoEnabled()) {
				logger.info("Loading Quartz config from ["
						+ this.configLocation + "]");
			}
			PropertiesLoaderUtils.fillProperties(mergedProps,
					this.configLocation);
		}

		CollectionUtils.mergePropertiesIntoMap(this.quartzProperties,
				mergedProps);
	...
	protected Scheduler createScheduler(SchedulerFactory schedulerFactory,
		...
		if (overrideClassLoader) {
			currentThread.setContextClassLoader(this.resourceLoader
					.getClassLoader());
		}
		try {
	...
	public Class<? extends Scheduler> getObjectType() {
		return (this.scheduler != null) ? this.scheduler.getClass()
				: Scheduler.class;
	}
	...
	public void stop() throws SchedulingException {
		if (this.scheduler != null) {
			try {
				...
				throw new SchedulingException(
						"Could not stop Quartz Scheduler", ex);
			}
		}
	}
