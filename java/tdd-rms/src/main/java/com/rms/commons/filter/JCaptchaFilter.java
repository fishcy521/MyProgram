...
import com.rms.modules.utils.web.ServletUtils;

/**
 * 集成JCaptcha验证码的Filter.
 * 
 * 可通过配置与SpringSecurity相同的登录表单处理URL与身份验证失败URL,实现与SpringSecurity的集成. 另本filter主要演示与SpringSecurity的集成方式，用户可参考其他验证码方案的集成.
 * 
 * 在web.xml中配置的参数包括： 1.failureUrl -- 身份验证失败后跳转的URL, 需与SpringSecurity中的配置保持一致, 无默认值必须配置. 2.filterProcessesUrl -- 登录表单处理URL, 需与SpringSecurity中的配置一致, 默认为"/j_spring_security_check". 3.captchaServiceId --
 * captchaService在Spring ApplicationContext中的bean id,默认为"captchaService". 4.captchaParamter -- 登录表单中验证码Input框的名称, 默认为"j_captcha". 5.autoPassValue -- 用于自动化功能测试的自动通过值, 默认该值不存在.
 * 
 * 
 */
public class JCaptchaFilter implements Filter {
	private static Log logger = LogFactory.getLog(JCaptchaFilter.class);
	...
	protected void initParameters(final FilterConfig fConfig) {
		...
		invalidUrl = fConfig.getInitParameter("invalidUrl");
	...
	public void doFilter(final ServletRequest theRequest, final ServletResponse theResponse, final FilterChain chain) throws IOException, ServletException {
		...
		if (StringUtils.startsWith(servletPath, filterProcessesUrl)) {
			...
			if (validated) {
				...
				/*if (accountManager.isVisitWeb(userName)) {
					chain.doFilter(request, response);
				} else {
					response.sendRedirect(request.getContextPath() + invalidUrl);
				}*/
			} else {
	 ...
	 * 生成验证码图片.
	 */
	protected void genernateCaptchaImage(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
	...
	protected boolean validateCaptchaChallenge(final HttpServletRequest request) {
		try {
			...
			System.out.println("开始验证登陆码：SessionID=" + captchaID + ";paramName=" + challengeResponse);
