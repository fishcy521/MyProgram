...
import com.rms.entity.IdEntity;
import com.rms.modules.utils.reflection.ReflectionUtils;
import com.rms.utils.BeanUtils;
...
public class HibernateUtils {
	...
	public static <T> void mergeByCheckedIds(final Collection<T> srcObjects,
			final Collection<Long> checkedIds, final Class<T> clazz,String fieldIdName) {
		...
		try {
			...
			while (srcIterator.hasNext()) {
				...
				//Field field = BeanUtils.getDeclaredField(element, fieldIdName);
				//Long id=(Long)field.get(element);
				Long id=(Long)PropertyUtils.getProperty(element, fieldIdName);
			...
			for (Long id : checkedIds) {
				T element = clazz.newInstance();
				PropertyUtils.setProperty(element, fieldIdName, id);
				//Field field = BeanUtils.getDeclaredField(element, fieldIdName);
