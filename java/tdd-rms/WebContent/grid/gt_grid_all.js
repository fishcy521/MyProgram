...
Sigma.init = function(_) {
	...
	Sigma.isBoxModel = Sigma.isIE && !Sigma.isIE8 && !Sigma.isIE9
			&& !Sigma.isStrict;
				...
				Sigma,
				{
					...
					$type : function($) {
						...
						if (_ == "object" && $.nodeType && $.nodeName)
							...
							case 1:
								return "element";
							case 3:
								return (/\S/).test($.nodeValue) ? "textnode"
										: "whitespace"
							}
						if (Sigma.U.isArray($))
							return "array";
						...
						else if (_ == "function" && typeof $.length == "number"
								&& $[0] !== undefined)
							return "collection";
						return _
					},
					...
					$array : function(B, _, D, A) {
						...
						if (B) {
							...
							if (!Sigma.$chk(D))
								D = B.length;
							if (Sigma.$type(B, "arguments", "collection")
									|| Sigma.$type(B, "array")
									&& (_ > 0 || D < B.length)) {
								for (var E = _; E < D; E++)
									$.push(B[E])
					...
					$clone : function(_, A) {
						...
						if (!_)
							$ = _;
						else if (Sigma.$type(_, "array", "arguments",
								"collection"))
					...
					$each : function(C, E, D, $) {
						var A = [];
						if (Sigma.$type(C, "array", "arguments", "collection")
								|| C && !Sigma.$type(C, "string")
								&& Sigma.$type(C.length, "number")) {
					...
					$getText : function($) {
						return $.innerText === undefined ? $.textContent
								: $.innerText
					},
					$element : function($, B) {
						if (Sigma.$type($, "string")) {
							if ($ == "input" && B) {
								$ = Sigma.doc.createElement("input");
								if (B.name) {
									$.name = B.name;
								}
								if (B.type) {
...
Sigma.Class = function(_) {
	...
	var $ = function() {
		...
		return (arguments[0] !== Sigma.$empty && Sigma.$type(this.initialize,
...
Sigma.$byId = function($, A) {
	...
	if (!$)
		return null;
	if (Sigma.U.contains([ "object", "embed" ], !$.tagName ? $.tagName
			.toLowerCase() : ""))
		return $;
	return Sigma.$e($)
};
...
Sigma.$byName = function(_) {
	...
	for (var B = 0; B < $.length; B++) {
		_ = $[B];
		A
				.push(Sigma.U.contains([ "object", "embed" ], _.tagName
						.toLowerCase()) ? _ : Sigma.$e(_))
	}
	return A
};
Sigma.$ = function($) {
...
Sigma.Utils = {
	...
	parseExpression : function(ex, pName, argNames, pStart, pEnd) {
		...
		while (true) {
			var start = ex.indexOf(pStart, current), sBegin = start
					+ startLength, sEnd = ex.indexOf(pEnd, sBegin), str = null, val = null;
	...
	removeClass : function($, _) {
		if ($)
			$.className = Sigma.U.clean($.className.replace(new RegExp(
					"(^|\\s)" + _ + "(?:\\s|$)"), "$1"));
		return $
	},
	toggleClass : function($, _) {
		return Sigma.U.hasClass($, _) ? Sigma.U.removeClass($, _) : Sigma.U
				.addClass($, _)
	},
	...
	createSelectHTML : function(B, D, G) {
		G = G || {};
		var $ = G.id ? (" id=\"" + G.id + "\" ") : " ", _ = G.className || "", C = G.style ? (" style=\""
				+ G.style + "\" ")
	...
	getBorderWidths : function($) {
		return [ Sigma.U.parseInt($.style.borderTopWidth),
				Sigma.U.parseInt($.style.borderRightWidth),
				Sigma.U.parseInt($.style.borderBottomWidth),
				Sigma.U.parseInt($.style.borderLeftWidth) ]
	},
	getPaddings : function($) {
		return [ Sigma.U.parseInt($.style.paddingTop),
				Sigma.U.parseInt($.style.paddingRight),
				Sigma.U.parseInt($.style.paddingBottom),
				Sigma.U.parseInt($.style.paddingLeft) ]
	},
	getPageX : function($) {
		$ = $ || window.event;
		var _ = $.pageX;
	...
	getScroll : function(A) {
		var $ = A, B = Sigma.doc;
		if ($ == B || $ == B.body) {
			var C = window.pageXOffset || B.documentElement.scrollLeft
					|| B.body.scrollLeft || 0, _ = window.pageYOffset
					|| B.documentElement.scrollTop || B.body.scrollTop || 0;
			return [ C, _ ]
		} else
	...
	getXY : function(F, A) {
		...
		while (H) {
			...
			if (Sigma.isGecko) {
				E = H;
				var C = parseInt(E.style.borderTopWidth, 10) || 0, G = parseInt(
						E.style.borderLeftWidth, 10) || 0;
				K += G;
				J += C;
		...
		while (H && H != I) {
			if (!Sigma.isOpera
					|| (H.tagName.toUpperCase() != "TR" && H.style.display != "inline")) {
				K -= H.scrollLeft;
				J -= H.scrollTop
			}
			H = H.parentNode
	...
	translatePoints : function(_, E, D) {
		...
		var A = _.style.position, B = Sigma.U.getXY(_), C = parseInt(
				_.style.left, 10), $ = parseInt(_.style.top, 10);
		if (isNaN(C))
	...
	getContentWidthHeight : function(A) {
		var _ = Sigma.U.parseInt(A.style.marginLeft), B = Sigma.U
				.parseInt(A.style.marginRight), D = Sigma.U
				.parseInt(A.style.paddingLeft), C = Sigma.U
				.parseInt(A.style.paddingRight), $ = A.clientWidth - D - C, E = A.clientHeight;
		return [ $, E ]
	},
	...
	replaceAll : function(exstr, ov, value) {
		...
		if (!Sigma.$chk(gc) || gc === "")
			return exstr;
		var rep = "/" + gc + "/gm", r = null, cmd = "r=exstr.replace(" + rep
				+ "," + Sigma.U.escapeString(value) + ")";
		eval(cmd);
		return r
	},
	trim : function(_, $) {
	...
	escapeRegExp : function($) {
		return !$ ? "" + $ : ("" + $).replace(/\\/gm, "\\\\").replace(
				/([\f\b\n\t\r[\^$|?*+(){}])/gm, "\\$1")
	},
	escapeString : function($) {
		return $ === "" ? "\"\"" : (!$ ? "" + $ : ("\""
				+ ("" + $).replace(/(["\\])/g, "\\$1") + "\"").replace(/[\f]/g,
	...
	return {
		createStyleSheet : function(C, $, A) {
			...
			if (Sigma.isIE) {
				...
				E.appendChild(F);
				D = F.styleSheet ? F.styleSheet
						: (F.sheet || A.styleSheets[A.styleSheets.length - 1])
			}
		...
		.$class({
			...
			send : function(I) {
				...
				if (this.queryParameters
						&& Sigma.$type(this.queryParameters, "object"))
			...
			isScript : function() {
				return (/(ecma|java)script/).test(this
...
Sigma.JSON = {
	encode : function(A, $, B) {
		...
		case "string":
			return "\""
					+ A.replace(/[\x00-\x1f\\"]/g, Sigma.JSON.$replaceChars)
					+ "\"";
		case "array":
			_ = [];
	...
	$replaceChars : function($) {
		return Sigma.JSON.$specialChars[$] || "\\u00"
...
Sigma.DataSetDefault = {
	...
	getRecordType : function(_, $) {
		...
		if (!Sigma.$type(this.recordType, "string")
	...
	updateRecord : function($, F, C) {
		...
		if (this.insertedRecords[B]
				|| Sigma.$invoke(this, "onRecordUpdate", [ $, F, C ]) !== false) {
			if (E == "int") {
				C = parseInt(C);
				C = isNaN(C) ? "" : C
			} else if (E == "float") {
	...
	sort : function(J) {
		...
		for (var D = 0; D < E.length; D++) {
			...
			if (H) {
				...
				C.push(!I && H.sortFn ? H.sortFn : this.getSortFuns($,
						H.sortOrder, B, H.getSortValue))
			}
		}
		...
		.$extend(
				Sigma.Template,
				{
					Grid : {
						main : function(_) {
							var $ = _.id, A = [
									_.toolbarPosition == "top"
											|| _.toolbarPosition == "t" ? "<div id=\""
											+ $
									...
									"<div id=\""
											...
											+ (_.simpleScrollbar ? " gt-simple-scrollbar"
													: "") + "\" >",
									"<div id=\""
											+ $
									...
									"<div id=\""
											+ $
											+ "_columnMoveS\" class=\"gt-column-moveflag\"></div>",
									"<div id=\""
											+ $
											+ "_headerGhost\" class=\"gt-head-ghost\"></div>",
									"</div>",
									"<div id=\""
											+ $
											+ "_bodyDiv\" class=\"gt-body-div\"></div>",
									"<div id=\""
											+ $
											+ "_freeze_headDiv\" class=\"gt-freeze-div\" ></div>",
									"<div id=\""
											+ $
											+ "_freeze_bodyDiv\" class=\"gt-freeze-div\" ></div>",
									"</div>",
									_.toolbarPosition == "bottom"
											|| _.toolbarPosition == "b" ? "<div id=\""
											+ $
											+ "_toolBarBox\" class=\"gt-toolbar-box\" ></div>"
											: "",
									"<div id=\""
											+ $
											+ "_separateLine\" class=\"gt-split-line\"></div>",
									"<div id=\""
											+ $
											+ "_mask\" class=\"gt-grid-mask\" >",
									"<div  id=\""
											+ $
											+ "_waiting\" class=\"gt-grid-waiting\">",
									"<div class=\"gt-grid-waiting-icon\"></div><div class=\"gt-grid-waiting-text\">"
											+ _.getMsg("WAITING_MSG")
											+ "</div>", "</div>",
						...
						createHeaderCell : function(B, $, _) {
							...
							A.innerHTML = [
									"<div class=\"gt-inner"
											+ ($.headAlign ? " gt-inner-"
													+ $.headAlign : "") + "\" ",
									_ ? "style=\"padding-top:0px;padding-bottom:0px;height:1px;\" "
											: "",
									"unselectable=\"on\" title=\"" + $.title
											+ "\" >", "<span>", C, "</span>",
									_ ? "" : Sigma.T_G.hdToolHTML, "</div>" ]
									.join("");
							return A
						},
						...
						rowStartS : function(_, A, $) {
							return [ "<tr class=\"gt-row",
									(A % 2 === 0 ? _.evenRowCss : ""), "\" ",
									Sigma.Const.DataSet.INDEX, "=\"", A, "\" ",
									$ ].join("")
						},
						rowEndHTML : "</tr>\n",
						innerStart : function($) {
						...
						cellEndHTML : "</div></td>",
						cell : function($, A, _) {
							return [ "<td ", _ || $.cellAttributes,
									" class=\"" + $.styleClass + "\" >",
									$.innerStartHTML, A, "</div></td>" ]
									.join("")
						},
						getColStyleClass : function($, _) {
							return (Sigma.Const.Grid.COL_T_CLASSNAME + $.id
									+ "-" + _).toLowerCase()
						},
						freezeBodyCell : function(D, C, _, A) {
							var F = C + D.cellWidthFix, $ = C + D.innerWidthFix, E = "style=\"width:"
									+ $ + "px;\"";
							_ = _ || "&#160;";
							var B = Sigma.$e("td", {
								style : {
					...
					Dialog : {
						create : function(C) {
							var $ = C.domId, A = C.gridId, B = Sigma
									.$chk(C.hasCloseButton) ? C.hasCloseButton
									: true, _ = C.title || "Dialog";
							return [
									...
									"</div><div id=\""
											+ $
											+ "_dialog_body\" class=\"gt-dialog-body\"></div>" ]
									.join("")
						},
						filterCondSelect : [
...
Sigma.GridDefault = {
	...
	domList : [ "gridWaiting", "separateLine", "gridMask", "gridGhost",
			"gridFormTextarea", "gridFormFileName", "gridFormExportType",
			"gridForm", "gridIFrame", "activeDialog", "gridDialog",
			"gridDialogTitle", "gridDialogBody", "gridFilterRSCache",
			"titleBar", "toolTipDiv", "freezeHeadTable", "freezeHeadDiv",
			"freezeBodyDiv", "freezeView", "resizeButton", "pageSizeSelect",
			"pageStateBar", "toolBar", "toolBarBox", "columnMoveS",
			"headerGhost", "headFirstRow", "bodyFirstRow", "headTable",
			"headDiv", "bodyDiv", "gridDiv", "viewport", "container" ],
	defaultConst : [ "action", "recordType", "exportType", "exportFileName",
			"exception", "parameters", "queryParameters", "data", "pageInfo",
			"filterInfo", "sortInfo", "columnInfo", "fieldsName",
			"insertedRecords", "updatedRecords", "updatedFields",
			"deletedRecords", "success", "succeedData", "failedData",
	...
	initialize : function(_, H) {
		...
		if (!this.dataset && this.columns) {
			...
			for (var C = 0; C < this.columns.length; C++) {
				var A = this.columns[C], $ = {
					...
					type : A.type,
					index : (A.fieldIndex || A.fieldIndex === 0) ? A.fieldIndex
							: null
				};
				D.fields.push($)
			}
		...
		if (this.dataset && !(this.dataset instanceof Sigma.DataSet)) {
			this.dataset.recordType = this.dataset.recordType
					|| this.recordType;
		...
		this.toolbarContent = this.toolbarContent === false ? false
				: this.toolbarContent;
		if (this.toolCreator)
	...
	initColumns : function(E) {
		...
		for (var G = 0; G < H; G++) {
			...
			_.editor = Sigma.Editor ? Sigma.Editor.factroy(_.editor, this)
					: null;
			...
			if (_.renderer == "by editor" && _.editor)
				...
				_.renderer = Sigma.U.parseExpression(_.renderer, "record",
	...
	initHead : function() {
		...
		this.freezeBodyDiv.style.top = this.headDivHeight + this.freezeFixH + 0
				+ "px"
	},
	initCSS : function() {
		var $ = "display:none;";
		...
		Sigma.$each(this.columnList, function(B, D) {
			...
			if (C.indexOf("px") < 1 && C.indexOf("%") < 1)
				...
				;
			_[D] = [
					B.CLASS_PREFIX + B.styleClass + " { width:"
							+ (C + A.cellWidthFix) + "px;"
							+ (B.hidden ? $ : "") + " } ",
					B.CLASS_PREFIX + B.innerStyleClass + " { width:"
							+ (C + A.innerWidthFix) + "px; } " ].join("\n")
		});
	...
	createMain : function() {
		...
		if (this.isNative)
			...
			var B = [
					(Sigma.isBoxModel ? "gt-b-ie "
							: (Sigma.isSafari ? "gt-b-safari "
									: (Sigma.isOpera ? "gt-b-opera "
			...
			if (!this.container || !this.container.appendChild
				...
				this.container.parentNode.insertBefore(this.gridDiv,
						this.container);
		...
		this.toolBarTopHeight = this.toolbarPosition == "top"
				|| this.toolbarPosition == "t" ? this.toolbarHeight
				+ (Sigma.isBoxModel ? 0 : 1) : 0;
	...
	_onResize : function(A) {
		...
		var B = ("" + this.height).indexOf("%") > 0 ? this.gridDiv.clientHeight
				: parseInt(this.height), _ = 0 - (Sigma.isBoxModel ? 2 : 3);
		this.bodyDiv.style.height = B
				- (this.headDivHeight + this.toolbarHeight) + _ + "px";
		if (Sigma.isOpera) {
	...
	createFormIFrame : function() {
		Sigma.U.createElementFromHTML(Sigma.T_G.formIFrame(this),
				Sigma.doc.body);
	...
	createHeader : function() {
		...
		var $ = this.headDiv.firstChild ? String(Sigma.U
				.getTagName(this.headDiv.firstChild)) : null;
		if ($ == "DIV" || $ == "SPAN")
	...
	createCustomHeader : function() {
		...
		for (var G = 0; G < this.headTbody.rows.length; G++) {
			...
			for (var C = 0; C < F.cells.length; C++) {
				var _ = F.cells[C], E = _.innerHTML, B = _
						.getAttribute("columnId");
				if (B) {
					_.columnId = B;
				...
				_.innerHTML = [ "<div class=\"gt-inner",
						_.rowSpan < 2 ? "" : " gt-inner-tall2",
						"\" unselectable=\"on\" >", "<span>", E, "</span>",
						B ? Sigma.T_G.hdToolHTML : "", "</div>" ].join("")
			}
		}
		D = Sigma.$e("tr", {
	...
	scrollGrid : function(I, H) {
		var C = this.tableMarginLeft, _ = this.freezeBodyDiv.clientWidth, $ = this.activeCell.offsetLeft
				+ ((Sigma.isFF2 || Sigma.isFF1) ? 0 : C), B = $
				+ this.activeCell.offsetWidth, F = this.activeCell.offsetTop, D = F
				+ this.activeCell.offsetHeight, A = this.bodyDiv.scrollLeft, E = this.bodyDiv.scrollTop, J = A
				+ this.bodyDiv.clientWidth + C, G = E
	...
	startEdit : function() {
		if (!this.readOnly
				&& this.activeCell
				&& this.activeEditor
				&& (this.activeColumn.editable || this
						.isInsertRow(this.activeRow))
				&& Sigma.$invoke(this.activeColumn, "beforeEdit", [
						this.activeValue, this.activeRecord, this.activeColumn,
						this ]) !== false
				&& Sigma.$invoke(this, "beforeEdit", [ this.activeValue,
						this.activeRecord, this.activeColumn, this ]) !== false
	...
	showEditor : function(B, $) {
		...
		if (this.activeEditor !== this.activeDialog) {
			this.activeEditor.setPosition(
					((Sigma.isFF2 || Sigma.isFF1) ? 0 : C) + A.offsetLeft,
					A.offsetTop);
	...
	hideEditor : function() {
		if (this.editing) {
			var B = this.activeRow, _ = this.activeValue, $ = this.activeEditor
					.parseValue(this.activeEditor.getValue()), A = this
					.validValue(this.activeColumn, $, this.activeRecord,
							this.activeCell);
			...
			Sigma.$invoke(this.activeColumn, "afterEdit", [ $, _,
					this.activeRecord, this.activeColumn, this ]);
			Sigma.$invoke(this, "afterEdit", [ $, _, this.activeRecord,
					this.activeColumn, this ])
		}
		if (this.activeEditor && this.activeEditor instanceof Sigma.Dialog)
			;
		else
	...
	getTwinRows : function(C) {
		var _ = C, B = C.rowIndex, $ = !_.id ? this.gridTbodyList[0]
				: this.freezeBodyTable.tBodies[0], A = $ ? $.rows[B] : null;
	...
	initActiveObj_startEdit : function(_, $, A) {
		...
		if (this.activeEditor
				&& (this.clickStartEdit || B && B == this.activeCell)) {
			Sigma.U.stopEvent(_);
			this.syncActiveObj($);
			this.startEdit()
		}
	...
	_onCellSelect : function($) {
		Sigma.$invoke(this, "onCellSelect", [ this.activeValue,
				this.activeRecord, $.cell, $.row, $.colNo, $.rowNo, $.column,
				this ])
	},
	...
	_onCellClick : function(_, A, $) {
		Sigma.$invoke(this, "onCellClick", [ $.value, $.record, $.cell, $.row,
				$.colNo, $.rowNo, $.column, this, _ ])
	},
	_onCellDblClick : function(_, A, $) {
		Sigma.$invoke(this, "onCellDblClick", [ $.value, $.record, $.cell,
				$.row, $.colNo, $.rowNo, $.column, this, _ ])
	},
	_onRowClick : function(_, A, $) {
		Sigma.$invoke(this, "onRowClick", [ $.value, $.record, $.cell, $.row,
				$.colNo, $.rowNo, $.column, this, _ ])
	},
	_onRowDblClick : function(_, A, $) {
		Sigma.$invoke(this, "onRowDblClick", [ $.value, $.record, $.cell,
				$.row, $.colNo, $.rowNo, $.column, this, _ ])
	},
	...
	_eventHandler : function(A, B, _) {
		...
		switch (B) {
		case "contextmenu":
			Sigma.$invoke(C, "onContextMenu", [ $.value, $.record, $.cell,
					$.row, $.colNo, $.rowNo, $.column, C ]);
			break;
		case "dblclick":
			this.clickCount = 0;
		...
		case "mouseover":
			Sigma.$invoke(C, "onMouseOver", [ $.value, $.record, $.cell, $.row,
					$.colNo, $.rowNo, $.column, C ]);
			break;
		case "mousemove":
			Sigma.$invoke(C, "onMouseMove", [ $.value, $.record, $.cell, $.row,
					$.colNo, $.rowNo, $.column, C ]);
			break;
		case "mouseout":
			Sigma.$invoke(C, "onMouseOut", [ $.value, $.record, $.cell, $.row,
					$.colNo, $.rowNo, $.column, C ]);
			break;
		default:
	...
	showCellToolTip : function(_, $) {
		...
		this.toolTipDiv.style.left = _.offsetLeft + this.bodyDiv.offsetLeft
				- this.bodyDiv.scrollLeft
				+ ((Sigma.isFF2 || Sigma.isFF1) ? 0 : this.tableMarginLeft)
				+ "px";
		this.toolTipDiv.style.top = _.offsetTop + _.offsetHeight
				+ this.bodyDiv.offsetTop - this.bodyDiv.scrollTop
				+ this.toolBarTopHeight + (Sigma.isFF ? 1 : 0) + "px";
	...
	getSaveParam : function($) {
		...
		this.submitUpdatedFields
				&& ($[this.CONST.updatedFields] = this.getUpdatedFields());
		return $
	},
	getLoadParam : function($) {
		...
		this.submitColumnInfo
				&& ($[this.CONST.columnInfo] = this.getColumnInfo());
	...
	load : function(B, D) {
		...
		if (Sigma.$invoke(this, "beforeLoad", [ _, this ]) !== false) {
			if (!E
					|| (D !== true && this.remotePaging === false && !this.isFirstLoad)) {
				C.requesting = true;
	...
	saveSuccess : function($, _) {
		...
		if (this.reloadAfterSave || this.autoSaveOnNav && _) {
			if (this.recountAfterSave)
				...
				this.getPageInfo().totalRowNum = $[$.CONST.pageInfo].totalRowNum
	...
	loadCallBack : function($, _) {
		...
		if (this.requesting) {
			var A = Sigma.$extend({}, B);
			if (A[this.CONST.success] === false
					|| A[this.CONST.success] === "false") {
				this.loadFailure(A);
				this.hideWaiting()
			} else
				this.loadSuccess(A);
	...
	responseCallBack : function(response, reqParam, action) {
		...
		response = Sigma.$invoke(this, action + "ResponseHandler", [ response,
				reqParam ])
				|| response;
	...
	renderInsertedRow : function($) {
		...
		A = Sigma.U.createTrFromHTML(this.buildGridRow(_, $, this.rowNum, D),
				this.gridTable.tFoot);
		if (this.showIndexColumn)
			C = Sigma.U.createTrFromHTML(this.buildGridIndexRow(_, $,
					this.rowNum, D), this.freezeBodyTable.tFoot);
		else {
			C = Sigma.U.createTrFromHTML(_ + "</tr>",
		...
		A.id = B;
		A.setAttribute(Sigma.Const.DataSet.INDEX,
				$[Sigma.Const.DataSet.SN_FIELD]);
		return [ A, C ]
	},
	hasDataRow : function() {
	...
	buildGridTable : function(E, I, _) {
		...
		A = (A < 2 ? 1.5 : A) * 7 + 2 + 1;
		var G = A + this.cellWidthFix, $ = A + this.innerWidthFix, J = "style=\"width:"
				+ G + "px;\"", F = "style=\"width:" + $ + "px;\"";
		this.indexColumnCell = [
				"<td class=\"gt-index-col\" " + J
						+ " ><div class=\"gt-inner\" " + F + " >",
				"</div></td>" ];
		if (E.showIndexColumn) {
			...
			E.freezeHeadDiv.style.left = E.freezeBodyDiv.style.left = this.freezeFixW
					+ "px";
			E.headTable.style.marginLeft = E.tableMarginLeft + "px";
			E.freezeHeadDiv.style.display = E.freezeBodyDiv.style.display = "block";
			E.freezeBodyDiv.style.height = parseInt(E.bodyDiv.style.height)
					+ "px"
		} else
			E.freezeHeadDiv.style.display = E.freezeBodyDiv.style.display = "none";
		_.push(Sigma.T_G.tableStartHTML);
		I.push(Sigma.U.replaceAll(Sigma.T_G.tableStartHTML, "margin-left:0px",
				"margin-left:" + E.tableMarginLeft + "px"));
	...
	buildGridRow : function(A, $, F, H, C) {
		...
		for (var B = 0; B < H; B++) {
			...
			E.push(Sigma.T_G.cell(_, _.renderer($[_.fieldIndex], $, _, this, G,
					F), D));
			G++
		}
	...
	buildGroupGridRow : function(_, $, C, D) {
		var B = [ _ ], A = "<td colspan=\"" + D
				+ "\" class=\"gt-group-row\" > + " + C + " -------------</td>";
		B.push(A);
		B.push(Sigma.T_G.rowEndHTML);
		return B.join("")
	},
					...
					for ( var _ in G) {
						...
						for (H = 0, D = this.columnList.length; H < D; H++) {
							...
							if (_ == $.fieldIndex && F.cells) {
								...
								F.cells[H].firstChild.innerHTML = $.renderer(
										C[$.fieldIndex], C, $, this, H,
										F.rowIndex)
							}
						}
					}
	...
	initToolbar : function() {
		...
		this.createGridMenu();
		if (this.toolbarContent && this.toolbarPosition
			...
			for (var E = 0; E < C.length; E++) {
				...
				if (A == "|") {
					...
					if (!this.pageStateBar)
						this.pageStateBar = Sigma.ToolFactroy.create(this,
								"pagestate", this.showPageState);
					if (E != C.length - 1)
					...
					$ = this.tools[A + "Tool"] = Sigma.ToolFactroy.create(this,
							A, this["show" + B + "Tool"])
				}
			}
		}
		...
		if (this.over_initToolbar) {
			...
			if (this.pageStateBar) {
				...
				if ($.endRowNum - $.startRowNum < 1)
					this.pageStateBar.innerHTML = "<div>"
							+ this.getMsg("NO_DATA") + "</div>";
				else
					this.pageStateBar.innerHTML = "<div>"
							+ Sigma.$msg(this.getMsg(_ ? "PAGE_STATE"
									: "PAGE_STATE_FULL"), $.startRowNum,
									$.endRowNum, $.totalPageNum, $.totalRowNum,
									$.pageNum) + "</div>"
			}
		}
	},
	...
	getSelectedRecord : function() {
		return this
				.getRecordByRow(this.selectedRows[this.selectedRows.length - 1])
				|| this.activeRecord
	},
	getGridHTML : function(_, C) {
		var B = this.getGridHeadHTML();
		C = C || [ Sigma.T_G.bodyTableStart(this.id, false), B, "<tbody>" ];
		var $ = 0, D = this.dataset.getSize(), A = _ ? this.getPageInfo().pageSize
				: -1;
		C.push(this.getGridBodyHTML($, D, A, C, null, true));
		return C.join("")
	},
	getGridHeadHTML : function() {
		var _ = this.headTable.innerHTML, $ = _.toLowerCase().indexOf("<tr"), A = _
				.toLowerCase().lastIndexOf("</tr>");
		if (this.customHead)
	...
	getGridBodyHTML : function(K, G, D, P, C, F) {
		...
		for (var B = K; B < G; B++) {
			...
			if (!J)
				break;
			var $ = N.customRowAttribute(J, B, N) || "", A = Sigma.T_G
					.rowStartS(N, B, $);
			if (!F) {
	...
	getHiddenColumnStyle : function(A) {
		...
		Sigma.$each(_.columnList, function(_, B) {
			if (_.hidden === true || (A && _[A] === false)) {
				$.push(_.CLASS_PREFIX + _.styleClass
						+ " { display:none;width:0px; }");
				$.push(_.CLASS_PREFIX + _.innerStyleClass
						+ " { display:none;width:0px; }")
			}
		});
		var B = $.join("\n");
	...
	cleanTable : function(B) {
		...
		if (this.freezeBodyTable) {
			Sigma.U.removeNode(this.freezeBodyTable.tBodies[0],
					this.freezeBodyTable);
		...
		if (_) {
			...
			for (var D = 0, A = E.length; D < A; D++) {
				...
				if (!this.isEmptyRow(C)) {
					...
					if (Sigma.$invoke(this, "beforeRowSelect",
							[ $, C, B, this ]) !== false) {
						...
						this.activeRow = C;
						Sigma
								.$invoke(this, "afterRowSelect", [ $, C, B,
										this ])
					}
				}
			}
	...
	endEdit : function() {
		if (this.activeEditor && this.activeEditor.locked === true
				...
				&& this.activeDialog && !this.activeDialog.hidden)
			return false;
		if (this.activeCell
				&& this.activeEditor
				&& (this.activeColumn.editable || this
	...
	exportGrid : function(F, C, G, A, B) {
		...
		if (Sigma.$invoke(D, "beforeExport", [ F, C, G, A, B, D ]) !== false) {
			try {
				...
				A = A
						|| this.jsonParamName
						|| (this.ajax ? this.ajax.jsonParamName
								: Sigma.AjaxDefault.paramName);
				B = "export";
				if (this.html2pdf && F == "pdf") {
					this.gridFormTextarea.name = "__gt_html";
					var E = [ "<style type=\"text/css\">",
							this.getHiddenColumnStyle("exportable"), "</style>" ];
	...
	getCellValue : function(B, C) {
		var _ = this.getColumn(B), A = this.getRecord(C), $ = A ? A[_.fieldIndex]
				: null;
		return $
	},
	...
	getEventTargets : function(A, _) {
		...
		var F = null, G = -1, E = -1, D = null, $ = null, C = null, B = Sigma.U
				.getParentByTagName("td", _, A);
		if (B) {
			F = B.parentNode;
	...
	getRecord : function($) {
		...
		if (Sigma.U.isNumber($))
			...
			_ = this.selectedRows[this.selectedRows.length - 1]
					.getAttribute(Sigma.Const.DataSet.INDEX) / 1;
		else
			_ = 0;
	...
	hideMask : function() {
		...
		this.pageSizeSelect
				&& (this.pageSizeSelect.style.visibility = "inherit")
	},
	insert : function(_, E) {
		_ = _ || this.cloneDefaultRecord()
	...
	save : function(N) {
		if (this.endEdit() === false)
			return;
		var E = this.saveURL, I = this.getInsertedRecords(), J = this
				.getUpdatedRecords(), $ = this.getDeletedRecords(), A = (I
				&& I.length > 0 || J && J.length > 0 || $ && $.length > 0);
		if (!N && !A)
			...
			for (var G = 0, D = I.length; G < D; G++) {
				...
				for (var K = 0; K < this.columnList.length; K++) {
					...
					if (H.editor) {
						var L = _[H.fieldIndex], C = F[G] ? F[G].cells[H.colIndex]
								: null;
			...
			if (Sigma.$invoke(this, "beforeSave", [ B, this ]) !== false) {
				...
				return this.request(E, B, "json", function($) {
					...
					var A = {};
					A[M.CONST.exception] = " XMLHttpRequest Status : "
							+ _.status;
					M.saveFailure(A, $);
					M.hideWaiting()
				})
	...
	setContent : function(_) {
		...
		if (_[this.dataRoot]
				&& Sigma.$invoke(this, "beforeDatasetUpdate",
						[ _[this.dataRoot] ]) !== false) {
	...
	showDialog : function(D) {
		...
		case "filter":
			...
			break;
		case "chart":
			var _ = A.activeCell ? A.getRecordByRow(A.activeRow) : A
					.getRecord();
			if (!_)
				break;
			var C = [], B = "", $ = 300, E = 300;
			if (_) {
				...
				A.chart = new Sigma.Chart({
					swfPath : A.SigmaGridPath
							+ "/flashchart/fusioncharts/charts/",
					width : $ - 3,
					height : E - 23,
				...
				Sigma.$each(A.columnList, function($, A) {
					...
					if ($.inChart)
						C.push([ $.header || $.title, _[$.fieldIndex],
								$.chartColor || "66bbff" ])
				});
	...
	showMask : function($) {
		...
		this.pageSizeSelect
				&& (this.pageSizeSelect.style.visibility = "hidden")
	},
	...
	applyFilter : function($) {
		...
		if (this.tools.filterTool)
			if (this.filterInfo.length > 0)
				Sigma.U.addClass(this.tools.filterTool.itemIcon,
						"gt-tool-filtered");
			else
				Sigma.U.removeClass(this.tools.filterTool.itemIcon,
						"gt-tool-filtered");
	...
	moveColumn : function(_, G, A) {
		if (_ == G)
			return;
		var B = this, F = this.columnList.length, C = A !== true
				&& this.freezeHeadDiv.style.display == "block" ? this.frozenColumnList.length
				: 0;
		G = G < C ? C : G;
		var $ = false, E;
		if (G >= F) {
		...
		Sigma.U.insertNodeBefore(this.columnList[_].headCell,
				this.columnList[G].headCell);
	...
	focusCell : function(_) {
		if (_ && !this.isGroupRow(_)) {
			...
			if ($) {
				Sigma.U.removeClass(this.activeCell, "gt-cell-actived"
						+ (this.activeEditor ? "-editable" : ""));
				this.syncTwinRowCell(null, this.activeCell)
			}
			this.syncActiveObj(_);
			$
					&& Sigma.U.addClass(this.activeCell, "gt-cell-actived"
							+ (this.activeEditor ? "-editable" : ""));
	...
	refreshRow : function(C, _) {
		...
		for (var E = 0; E < C.cells.length; E++) {
			...
			A.firstChild.innerHTML = $.renderer(_[$.fieldIndex], _, $, this, E,
					B)
		}
	},
	checkRow : function(B, A) {
		...
		for (var C = 0; C < B.cells.length; C++) {
			...
			if ($.isCheckColumn) {
				...
				if (Sigma.U.getTagName(_) == "INPUT" && _.type == "checkbox"
						&& Sigma.U.hasClass(_, "gt-f-check")) {
					if (_.checked !== A) {
						_.checked = A;
						if (this.selectRowByCheck)
							this.selectRow(B, A);
						A ? this.checkedRows[_.value] = true
								: delete this.checkedRows[_.value]
					}
					return
				}
			}
		...
		.$extend(
				Sigma.Grid,
				{
					...
					handleOverRowCore : function($, _, A) {
						...
						if (_.overRow)
							_.overRow.className = _.overRow.className.replace(
									" gt-row-over", "");
					...
					startGridResize : function($, _) {
						...
						_.syncLeftTop();
						_.gridGhost.style.top = _.top
								+ Sigma.doc.body.scrollTop + "px";
						_.gridGhost.style.left = _.left
								+ Sigma.doc.body.scrollLeft + "px";
					...
					endGridResize : function(_, B) {
						var A = Sigma.U.parseInt(B.gridGhost.style.width)
								+ "px", $ = Sigma.U
								.parseInt(B.gridGhost.style.height)
								+ "px";
					...
					startColumnResize : function(_, $) {
						...
						B.separateLineMinX = $.oldRightX + $.minWidth
								- $.headCell.offsetWidth;
						if (B.separateLine) {
							B.separateLine.style.left = $.oldRightX + "px";
							B.separateLine.style.height = B.viewport.offsetHeight
									- 2 + "px";
							B.separateLine.style.display = "block"
						}
						if (B.columnMoveS) {
							B.columnMoveS.style.left = $.headCell.offsetLeft
									+ ((Sigma.isFF2 || Sigma.isFF1) ? 0
											: B.tableMarginLeft) + "px";
					...
					startColumnMove : function(A, $) {
						...
						C.mouseDown = true;
						if (!A.ctrlKey
								|| C.frozenColumnList[$.getColumnIndex()])
							return;
						C.closeGridMenu();
					...
					doDocGridHandler : function(C, G) {
						...
						if (G.separateLine
							...
							if (H <= B.offsetLeft)
								I = 0;
							if (_ >= 0) {
								G.columnMoveS.style.left = _
										+ ((Sigma.isFF2 || Sigma.isFF1) ? 0
												: G.tableMarginLeft) + "px";
							...
							var D = C.clientX - G.gridGhost.cx, $ = C.clientY
									- G.gridGhost.cy;
					...
					endDocGridHandler : function(A, E) {
						...
						E.mouseDown = false;
						if (E.separateLine
								&& E.separateLine.style.display == "block") {
							var $ = E.columnMap[E.resizeColumnId];
							$.newRightX = D - E.viewportXY[0];
							var C = $.newRightX - $.oldRightX, B = C
									+ parseInt($.width);
							$.setWidth(B);
							E.resizeColumnId = -1;
						...
						} else if (!E.customHead && E.headerGhost
								&& E.headerGhost.style.display == "block") {
							var F = Sigma.isIE ? A.x : A.pageX, G = E.columnMoveS
									.getAttribute("newColIndex"), _ = E.headerGhost
									.getAttribute("colIndex");
							if (G !== null && (G + "").length > 0 && _ !== null
									&& (_ + "").length > 0) {
								G = G / 1;
								if (G < 0)
									G = E.columnList.length;
								if (Sigma.$invoke(E, "beforeColumnMove", [
										_ / 1, G / 1, E ]) !== false) {
					...
					changeSkin : function(_, $) {
						...
						for (var B = 0; B < A.length; B++)
							if (A[B]
									.indexOf(Sigma.Const.Grid.SKIN_CLASSNAME_PREFIX) === 0)
								A[B] = "";
					...
					createCheckColumn : function(C, E) {
						var $ = E.id;
						C = Sigma.$grid(C);
						var B = C.id, D = E.checkValid, _ = E.checkValue, F = E.checkType
								|| "checkbox";
						if (!_)
							_ = Sigma.$chk(E.fieldIndex) ? "record[\""
									+ E.fieldIndex + "\"];"
									: "grid.getUniqueField(record);";
						if (typeof _ == "string")
							_ = new Function("value", "record", "col", "grid",
									"colNo", "rowNo", [ "return ", _ ].join(""));
						if (!D)
						...
						E.renderer = function(G, C, $, B, J, H) {
							var E = _(G, C, $, B, J, H), I = D(E, G, C, $, B,
									J, H) ? "checked=\"checked\"" : "";
							return "<input type=\"" + F
									+ "\" class=\"gt-f-check\" value=\"" + E
									+ "\" " + I + " name=\"" + A + "\" />"
						};
						return E
					}
		...
		.$extend(
				Sigma.Grid,
				{
					...
					initColumnEvent : function(B, $, C, A) {
						...
						var _ = Sigma.U.nextElement(A), D = Sigma.U
								.nextElement(_);
								...
								.addEvent(
										...
										function(A) {
											...
											if (!B.isColumnResizing) {
												Sigma.$invoke(B, "onHeadClick",
														[ A, C, $, B ]);
												if (Sigma.U.getTagName(D) == "INPUT"
														&& D.type == "checkbox"
														&& Sigma.U
																.hasClass(D,
																		"gt-f-totalcheck"))
													Sigma
														...
														&& D.className != "gt-hd-button") {
													B.lastAction = "sort";
													B.sorting = true;
													var _ = $.sortOrder == "asc" ? "desc"
															: ($.sortOrder == "desc"
																	&& $.enableDefaultSort ? "defaultsort"
																	: "asc"), E = B
															.createSortInfo($);
													E.sortOrder = _;
													B.addSortInfo(E);
					...
					findGridByElement : function(B) {
						...
						while ((B = B.parentNode))
							if (Sigma.U.getTagName(B) == _
									&& Sigma.U.hasClass(B, A)) {
								$ = B.id;
								break
							}
...
Sigma.checkOneBox = function(_, C, $) {
	...
	if (_.checked == $)
		return $;
	var B = Sigma.U.getParentByTagName("td", _), D = B.parentNode, A = C
			.getTwinRows(D)[0];
	if ($ === true || $ === false) {
...
Sigma.checkTotalBox = function(H, J, G, I) {
	...
	if (I !== null && I !== undefined)
		H.checked = I;
	var _ = Sigma.U.getParentByTagName("td", H), $ = Sigma.U.getCellIndex(_), K = H.checked, E = G.frozen ? J
			.getAllFreezeRows()
			: J.getAllRows();
...
Sigma.initGlobalEvent = function() {
	...
	Sigma.U.addEvent($, "click", function($) {
		Sigma.activeGrid
				&& (Sigma.activeGrid.endEdit() || Sigma.activeGrid
						.closeGridMenu())
	});
...
Sigma.ColumnDefault = {
	CLASS_PREFIX : ".",
	destroyList : [ "sortIcon", "hdTool", "menuButton", "separator",
			"frozenSortIcon", "frozenHdTool", "frozenHeadCell", "headCell",
			"firstCell" ],
	id : 0,
	fieldName : null,
	width : 120,
	...
	setWidth : function($) {
		...
		this.width = $ + "px";
		Sigma.U.CSS.updateRule(this.CLASS_PREFIX + this.styleClass, "width",
				($ + _.cellWidthFix) + "px");
		Sigma.U.CSS.updateRule(this.CLASS_PREFIX + this.innerStyleClass,
				"width", ($ + _.innerWidthFix) + "px")
	},
	setHeader : function(_) {
		this.header = _;
	...
	freezeCell : function(F, D, $, C, G, B, A, _) {
		...
		if (_ && C === 0) {
			...
			this.frozenSortIcon = Sigma.Grid.getSortIcon(this,
					this.frozenHeadCell);
			this.frozenHdTool = Sigma.Grid.getHdTool(this, this.frozenHeadCell);
			if (!Sigma.isIE)
				Sigma.Grid.initColumnEvent(A, this, this.frozenHeadCell,
						this.frozenSortIcon)
		}
	},
	freeze : function(B) {
		...
		for (E = 0; E < F.length; E++)
			this.freezeCell(F[E], A.freezeHeadTable.tBodies[0], _[E], E, G, $,
					A, true);
		if (A.rowNum < 1)
			;
		...
		for (E = 0; E < F.length; E++)
			this.freezeCell(F[E], A.freezeBodyTable.tBodies[0], _[E], E, G, D,
					A);
		if (!B) {
		...
		if (!A.hasIndexColumn)
			A.freezeHeadDiv.style.left = A.freezeBodyDiv.style.left = 0
					- (C + A.cellWidthFix) + A.freezeFixW + "px";
...
Sigma.Navigator = Sigma
		.$class({
			...
			destroy : function() {
				var $ = this, _ = [ "firstPageButton", "prevPageButton",
						"nextPageButton", "lastPageButton", "gotoPageButton" ];
			...
			createGotoPage : function() {
				...
				if (B.toolBar) {
					...
					B.toolBar.appendChild(Sigma.Button
							.createToolComp(this.pageInput));
			...
			refreshState : function($, B) {
				...
				if (B !== false) {
					...
					A.pageNum = A.pageNum > A.totalPageNum ? A.totalPageNum
							: A.pageNum;
					...
					A.endRowNum = A.endRowNum > A.totalRowNum ? A.totalRowNum
							: A.endRowNum
				}
				return A
			},
			refreshNavBar : function($) {
				...
				if (_ && _.pageSizeSelect) {
					_.pageSizeList = !_.pageSizeList
							|| _.pageSizeList.length < 1 ? [ _.pageSize ]
							: _.pageSizeList;
					_.pageSizeSelect.innerHTML = "";
					Sigma.U.createSelect(Sigma.U.listToMap(_.pageSizeList),
							this.pageInfo.pageSize, {}, _.pageSizeSelect)
				}
			},
			gotoPage : function(A, C, $) {
				...
				C = (!C || C < 1) ? 1
						: (C > A.pageInfo.totalPageNum ? A.pageInfo.totalPageNum
								: C);
			...
			gotoInputPage : function($, _) {
				_.gotoPage(_, Sigma.U.parseInt(_.pageInput.value,
						_.pageInfo.pageNum), "gotoPage")
			},
			...
			onclickAction : function(B, $) {
				...
				if ($.parentItem) {
					...
					if ($.parentItem.currenItem)
						Sigma.U.removeClass($.parentItem.currenItem.itemBox,
								"gt-menu-activemenu");
					$.parentItem.currenItem = $;
					Sigma.U.addClass($.itemBox, "gt-menu-activemenu")
				}
				if ($.disabled
						|| $.onclick.apply($, [ B ].concat($.onclickArgs || $)) === false) {
					Sigma.U.stopEvent(B);
					return
				}
				Sigma.U.stopEvent(B);
			...
			addMenuItems : function(_, $) {
				if (_) {
					if (!this.subMenu) {
						...
						this.itemAfterIcon
								&& Sigma.U.addClass(this.itemAfterIcon,
										"gt-menu-parent")
					}
...
Sigma.MenuItem = Sigma.BaseMenuItem.extend({
	initialize : function(_) {
		...
		if (this.type == "checkbox" || this.type == "radiobox")
			this.cls = this.checked ? ("gt-icon-" + this.type)
					: "gt-icon-unchecked";
		this.itemBox = Sigma.$e("a", {
			href : "javascript:void(0);return false;",
...
Sigma.ToolFactroy = {
	...
	tools : {
		...
		"pagesize" : function(C) {
			...
			C.toolBar
					.appendChild(Sigma.Button.createToolComp(C.pageSizeSelect));
...
Sigma.createColumnTable = function(C, $) {
	...
	function A($, _) {
		...
		return "<input type=\"" + $.checkType + "\" name=\"" + $.name
				+ "\" value=\"" + $.value(_) + "\" class=\"gt-f-check\" "
				+ ($.checked(_) ? " checked=\"checked\" " : "")
				+ (!A ? " disabled=\"disabled\" " : "") + " />"
	}
	function B($) {
		return $.checkType == "checkbox" ? "<input type=\"checkbox\" name=\""
				+ $.name + "\" class=\"gt-f-totalcheck\" />"
				: "<input type=\"radio\" name=\"" + $.name + "\" />"
	}
	var _ = C.columnList, D = [
			...
			"<thead>", Sigma.T_G.rowStart(C, 0), Sigma.T_G.cellStartHTML, B($),
			Sigma.T_G.cellEndHTML, Sigma.T_G.cellStartHTML,
			C.getMsg("COLUMNS_HEADER"), Sigma.T_G.cellEndHTML,
			Sigma.T_G.rowEndHTML, "</thead>", "<tbody>" ];
	for (var E = 0; E < _.length; E++)
		D.push([ Sigma.T_G.rowStart(C, E), Sigma.T_G.cellStartHTML, A($, _[E]),
				Sigma.T_G.cellEndHTML, Sigma.T_G.cellStartHTML,
				_[E].header || _[E].title, Sigma.T_G.cellEndHTML,
				Sigma.T_G.rowEndHTML ].join(""));
	D.push("</tbody></table>");
	return D.join("\n")
};
Sigma.checkChecked = function(D) {
	D = Sigma.$grid(D);
	var F = D.chkAll, _ = Sigma.U.getParentByTagName("td", F), $ = Sigma.U
			.getCellIndex(_), H = D.getAllRows(), C = 0;
...
Sigma.createColumnDialog = function(C, E) {
	var $ = C + "ColCheck", _ = E.gridId, G = _ + "_" + C + "ColDialog", D = Sigma
			.$grid(_), B = function() {
		var H = Sigma.$(G + "_div"), B = (Sigma.U.getTagName(H) == "TABLE") ? H
				: H.getElementsByTagName("table")[0], F = B.tBodies[0], A = F
				.getElementsByTagName("input"), C = Sigma.U.getCheckboxState(A,
				$), I = [], J;
		...
		Sigma.WidgetCache[G].close()
	}, F = new Sigma.Dialog(
			{
				...
				afterShow : function() {
					var A = Sigma.$grid(this.gridId), _ = Sigma
							.createColumnTable(this.gridId, {
								type : "checkbox",
								name : $,
								value : function($) {
									return $.id
...
Sigma.createFilterDialog = function(I) {
	...
	H.justShowFiltered = I.justShowFiltered === true ? true
			: (I.justShowFiltered === false ? false : H.justShowFiltered);
	...
	var A = function() {
		...
		for (var J = 0; J < C.length; J++)
			if (Sigma.U.getTagName(C[J]) == "DIV"
					&& C[J].className == "gt-filter-item") {
				var A = C[J].childNodes[1], B = C[J].childNodes[2], _ = C[J].childNodes[3].firstChild, G = Sigma.U
						.getValue(A), $ = H.columnMap[G];
				if ($ && $.fieldName)
					D.push({
						columnId : G,
						fieldName : $.fieldName,
		...
		Sigma.WidgetCache[K].close()
	}, $ = 430, E = 220, G = $ - (Sigma.isBoxModel ? 16 : 18), _ = E
			- (Sigma.isBoxModel ? 93 : 95), J = new Sigma.Dialog(
			{
				id : K,
				gridId : B,
				title : I.title,
				...
				afterShow : function() {
					...
					for (var I = 0; I < D.length; I++) {
						var G = D[I].columnId, A = E.getColumn(G), H = (A.header || A.title), C = Sigma
								.createFilterItem(E, G, H), _ = C.childNodes[1], B = C.childNodes[2], $ = C.childNodes[3].firstChild;
					...
					if (D.length < 1) {
						Sigma.$(this.id + "_div").innerHTML = "<div style=\"color:#999999;margin:10px;\">"
								+ E.getMsg("DIAG_NO_FILTER") + "</div>";
						E._noFilter = true
					}
				}
			});
...
Sigma.createFilterItem = function(C, D, E) {
	...
	}), A = "<input type=\"text\" readonly=\"readonly\" class=\"gt-input-text gt-filter-col-text\" value=\""
			+ E + "\" />";
	A += "<input type=\"hidden\"  value=\"" + D + "\" />";
	var $ = Sigma.createFilterField(C, D), _ = "<button class=\"gt-input-button gt-filter-del\" >"
			+ C.getMsg("TEXT_DEL")
			+ "</button>"
...
Sigma.clickHandler = {
	...
	onFilterItem : function(A) {
		A = A || window.event;
		var E = Sigma.U.getEventTarget(A), _ = Sigma.U.getParentByTagName(
				"table", null, A, 10);
	...
	onTotalCheck : function(A) {
		A = A || window.event;
		var D = Sigma.U.getEventTarget(A), _ = Sigma.U.getParentByTagName(
				"table", null, A, 10);
	...
	defaultValidator : function(H, D, C, I, E) {
		...
		for (var B = 0; B < K.length; B++) {
			...
			if (Sigma.$type($, "function"))
				A = $.apply($, J);
			if (A !== true) {
				var _ = Sigma.Validator.getMessage(this.validRule[B])
						|| String(A);
				F.push(_)
			}
		}
		...
		.$extend(
				Sigma.Editor,
				{
					factroy : function($, _) {
						if (Sigma.$type($, "function"))
							$ = $(_);
						if (($ instanceof Sigma.DialogEditor)
								|| ($ instanceof Sigma.Dialog)) {
							$.gridId = _.id;
						...
						return $ && Sigma.Editor.EDITORS[$.type] ? Sigma.Editor.EDITORS[$.type]
								($)
								: null
					},
					register : function(_, $) {
					...
					EDITORS : {
							...
							$.getDisplayValue = function($) {
								...
								for (var _ = 0; _ < this.valueDom.options.length; _++)
									if (String(this.valueDom.options[_].value) === String($))
										return this.valueDom.options[_].text
												|| this.valueDom.options[_].innerHTML;
								return (this.defaultText || this.defaultText === "") ? this.defaultText
										: null
							};
							return $
						},
						...
						date : function(_) {
							...
							_.setSize = function($, _) {
								...
								this.height = _ || this.height;
								if (this.width / 1 && this.width > 0)
									this.dom.style.width = (this.width - 1)
											+ "px";
								if (this.height / 1 && this.height > 0)
									this.dom.style.height = (this.height - 1)
											+ "px";
								this.dom.firstChild.style.width = (this.width - 20)
										+ "px"
							};
							var B = function($) {
								_.onClose && _.onClose();
...
Sigma.Validator = {
	...
	getMessage : function(msgKey) {
		...
		var _format = ((Sigma.Validator.KeyMapping[msgKey] || msgKey) + "_FORMAT")
				.toUpperCase();
	...
	"integer" : function($) {
		return (String($).indexOf(".") < 0) && !isNaN($ / 1)
				&& Sigma.Validator.RegExpLib["integer"].test(Math.abs($))
	},
	"float" : function($) {
		return !isNaN($ / 1)
				&& Sigma.Validator.RegExpLib["float"].test(Math.abs($))
	},
	"money" : function($) {
	...
	"date" : function(K, A) {
		...
		$ = "^" + $ + "$";
		var C = new RegExp($), J = 0, H = 1, D = 1, I = A
				.match(/(YYYY|YY|MM|M|DD|D)/g);
	...
	"datetime" : function(_, D) {
		...
		for (var A = 0; A < _.length; A++) {
			if (!C.test(_[A]))
				return false;
			var $ = _[A].split(" "), E = D.split(" "), B = Sigma.Validator
					.date($[0], E[0])
					&& Sigma.Validator.time($[1], E[1]);
			if (!B)
				return false
		}
		return true
		...
		.$class({
			initialize : function($) {
				...
				if (this.swfPath.lastIndexOf("/") == this.swfPath.length - 1)
					this.swfPath = this.swfPath.substring(0,
							this.swfPath.length - 1);
				this.container = Sigma.$(this.container);
				this.chart = this.chart
						|| new FusionCharts(this.swfPath + "/" + this.swf,
								this.container.id + "_chart", this.width,
								this.height)
			},
			...
			createSetsXML : function(C) {
				...
				for (var G = 0; G < C.length; G++) {
					...
					if ($ instanceof Array) {
						...
						F = $[1];
						B = $[2];
						B = (F === null || F === undefined) ? null
								: (B || this.defaultColor);
						D = (D === null || D === undefined) ? F : D;
						A = [
								D !== null && D !== undefined ? "name='" + D
										+ "'" : "",
								F !== null && F !== undefined ? "value='" + F
										+ "'" : "",
								B !== null && B !== undefined ? "color='" + B
										+ "'" : "" ].join(" ")
					} else if ($)
						A = this.json2Params($);
					_ = [ "<set", A, "/>" ];
			...
			createChartXML : function(A, $) {
				$ = $ || this.setsXML;
				var _ = [ "<graph", "caption='" + (this.caption || "") + "'",
						"subCaption='" + (this.subCaption || "") + "'",
						"outCnvBaseFontSize='12'", "animation='0'" ];
				_.push(">" + $ + "</graph>");
