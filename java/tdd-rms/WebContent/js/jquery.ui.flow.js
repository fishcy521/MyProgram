(function($){
	$.fn.flow = function(option){
		var defaultOptions = {
			...
			slider : 10,
			handle : "draw",
			nodeClassName:"module_link",
			img : {
				right : "/pms/images/flow/right_sword.png",
				up : "/pms/images/flow/right_sword.png"
			},
			hover : function(){
		...
		return this.each(function(){
			...
			var createObjectList = function(id,level){
				var root = {
					...
					getChild:function(i){
						return $("."+opts.nodeClassName+"[id=" + this.child[i].id + "]");
					}
				};
				var $rootObject = $("."+opts.nodeClassName+"[id=" + id + "]",currentQuery);
				...
				for(var i = 0;i < childID.length;i++){
					var child = $("."+opts.nodeClassName+"[id=" + childID[i] + "]",currentQuery);
					if(child.size() == 0){
						continue;	
					}
			...
			var rootID = $("."+opts.nodeClassName+"[begin]",currentQuery).attr("id");
			var myObjectList = createObjectList(rootID,0);
			var drawDiv = function(objectList,spanID,left,top,maxHeight){
				var span = $("#" + spanID);
				var $root = $("."+opts.nodeClassName+"[id=" + objectList.id + "]",currentQuery).clone();
				$("."+opts.nodeClassName+"[id=" + objectList.id + "]",currentQuery).remove();
				if($("."+opts.nodeClassName+"[id=" + objectList.id + "]").size() > 0){
					$root = $("."+opts.nodeClassName+"[id=" + objectList.id + "]").eq(0);
					return;
				}else{
					$("#" + opts.handle + " ."+opts.nodeClassName+"").each(function(){
			...
			try{
				...
				$("#" + opts.handle).find("."+opts.nodeClassName+"").hover(opts.hover,opts.remove).click(opts.click);
			}catch(e){
				alert(e.description)
			}
		});
